 ...
 * 使用统一SEO系统 - 支持页面属性和后端SEO字段
 * 
 * SEO策略: 核心词 + 分类词 + 页面属性词 + 后端seoTitle/seoDescription
 * 
 * 属性关键词来源:
 * - theme: "animals coloring pages"
 * - style: "simple coloring pages" 
 * - difficulty: "easy coloring pages"
 * - ageRange: "coloring pages for ages 2-12"
 */
export async function generateMetadata({ params }: CategoryDetailPageProps) {
  const { category, 'slug-id': slugId } = await params;
  return await generateCategoriesSEO('detail', { category, slugId });
}
