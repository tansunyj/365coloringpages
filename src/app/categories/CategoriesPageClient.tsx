...
export default function CategoriesPageClient() {
  ...
  const getCurrentCategoryDisplay = () => {
    if (params.category && typeof params.category === 'string') {
      return decodeURIComponent(params.category).replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    }
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#fcfcf8' }}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-8">
          ...
          <Link href="/categories" className="hover:text-yellow-600 transition-colors">
            Categories
          </Link>
          {params.category && (
            <>
              <ChevronRight className="h-4 w-4" />
              <span className="text-gray-900 font-medium">{getCurrentCategoryDisplay()}</span>
            </>
          )}
        </nav>
        ...
        <div className="mb-12">
          <h1 className="text-5xl font-extrabold text-gray-900 mb-6 leading-tight">
            {params.category ? `${getCurrentCategoryDisplay()} Coloring Pages` : 'All Coloring Pages'}
          </h1>
          <p className="text-xl text-gray-600 max-w-4xl leading-relaxed">
            {params.category 
              ? `Explore our beautiful collection of ${getCurrentCategoryDisplay().toLowerCase()} coloring pages.`
