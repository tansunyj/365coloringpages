...
const generateDefaultAvatar = (email: string, name?: string) => {
  // 使用UI Avatars服务生成漂亮的字母头�?
  const displayName = name || email.split('@')[0];
  // 使用邮箱的首字母，背景色使用橙黄色系，只显示首字�?
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        console.log('🔐 OAuth 登录成功，开始获取用户信�?..');
        console.log('Token:', token.substring(0, 20) + '...');
        console.log('Provider:', provider);
        
        // 保存 token �?localStorage
        localStorage.setItem('authToken', token);
        localStorage.setItem('token', token); // 兼容�?
        localStorage.setItem('authProvider', provider || 'unknown');
        
        // 使用 token 获取用户信息（根据接口文档使�?/api/user/me�?
        ...
        console.log('📡 用户信息接口响应状�?', response.status);
        ...
        if (data.success && data.data) {
          ...
          console.log('🖼�?头像URL:', { original: data.data.avatar, final: avatarUrl });
          ...
          console.log('💾 保存用户信息�?localStorage:', userInfo);
          localStorage.setItem('userInfo', JSON.stringify(userInfo));
          
          console.log('�?用户信息保存成功，准备跳�?..');
        } else {
