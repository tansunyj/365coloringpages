...
/**
 * Generate dynamic SEO metadata for search detail pages
 * Combines search keywords with category and page-specific data
 */
export async function generateMetadata({ params }: SearchDetailPageProps): Promise<Metadata> {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/coloring-pages/${id}`, fetchOptions);
  ...
  pageKeywords.push('search coloring pages');
  pageKeywords.push('find coloring pages');
  ...
  const title = pageData.seoTitle && pageData.seoTitle.length >= 30 && pageData.seoTitle.length <= 80
    ? pageData.seoTitle
    : `${pageData.title} - ${displayCategory.charAt(0).toUpperCase() + displayCategory.slice(1)} Coloring Page | Search Results`;
  ...
  if (pageData.seoDescription && pageData.seoDescription.length >= 100 && pageData.seoDescription.length <= 200) {
    ...
    description = `Search result: Download and print ${pageData.title} coloring page. `;
  ...
  const url = `${site}/search/${category}/${slugId}`;
    ...
    for (const cat of categories) {
      try {
        const pagesResponse = await fetch(
          `${API_BASE}/api/search?category=${cat.slug}&limit=1000`,
          { cache: 'no-store' }
        );
