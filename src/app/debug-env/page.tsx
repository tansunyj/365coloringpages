...
export default function DebugEnvPage() {
  ...
  return (
    <div className="container mx-auto p-8">
      <h1 className="text-2xl font-bold mb-6">🔍 环境变量调试页面 (增强版)</h1>
      ...
      <div className="bg-blue-50 p-6 rounded-lg mb-6">
        ...
        <div className="space-y-2 text-sm">
          <div><strong>构建模式:</strong> {typeof window !== 'undefined' ? '客户端' : '服务端'}</div>
          <div><strong>组件已挂载:</strong> {mounted ? '是' : '否'}</div>
          <div><strong>当前URL:</strong> {typeof window !== 'undefined' ? window.location.href : '服务端渲染'}</div>
          <div><strong>Next.js 数据:</strong> {typeof window !== 'undefined' && (window as any).__NEXT_DATA__ ? '已加载' : '未加载'}</div>
      ...
      <div className="bg-yellow-50 p-6 rounded-lg mb-6">
        <h2 className="text-lg font-semibold mb-4 text-yellow-800">📋 所有 NEXT_PUBLIC_ 环境变量：</h2>
      ...
      <div className="bg-green-50 p-6 rounded-lg">
        ...
        <div className="text-green-700">
          <div><strong>期望的 API 基础URL:</strong> http://localhost:3001</div>
          <div><strong>实际获取到的值:</strong> {apiBaseUrl || '(空值)'}</div>
          <div><strong>匹配状态:</strong> 
            <span className={`ml-2 px-2 py-1 rounded text-xs ${
              apiBaseUrl === 'http://localhost:3001' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
            }`}>
              {apiBaseUrl === 'http://localhost:3001' ? '✅ 匹配' : '❌ 不匹配'}
            </span>
          </div>
        </div>
      </div>
